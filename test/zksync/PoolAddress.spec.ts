...
import { computePoolAddress, POOL_BYTECODE_HASH } from './shared/computePoolAddress'
...
describe('PoolAddress', () => {
  ...
  const poolAddressTestFixture = async () => {
    ...
    const poolAddressTestFactory = await getContractFactory('PoolAddressTest')
  ...
  beforeEach('deploy PoolAddressTest', async () => {
    ...
    poolAddress = await poolAddressTestFixture()
  })

  describe('#POOL_INIT_CODE_HASH', () => {
    it('equals the hash of the pool bytecode', async () => {
      expect(await poolAddress.POOL_INIT_CODE_HASH()).to.eq(utils.hexlify(POOL_BYTECODE_HASH))
    })
  })
