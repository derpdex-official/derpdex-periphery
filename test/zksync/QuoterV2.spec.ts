...
import { FeeAmount, MaxUint128, PRIVATE_KEY } from './shared/constants'
...
describe('QuoterV2', function () {
  ...
  const swapRouterFixture = async (wallets: Wallet[], provider: Provider) => {
    ...
    quoter = (await getContractInstance("QuoterV2", [factory.address, weth9.address])) as QuoterV2
  ...
  beforeEach('load fixture', async () => {
    ...
    ;({ tokens, nft, quoter } = await swapRouterFixture([wallet], Provider.getDefaultProvider()))
  })
  ...
  describe('quotes', () => {
    beforeEach(async () => {
      ...
      await tx.wait()
      tx = await createPoolWithMultiplePositions(nft, wallet, tokens[0].address, tokens[2].address)
      await tx.wait()
    })
    ...
    describe('#quoteExactInput', () => {
      ...
      it('0 -> 2 cross 0 tick, starting tick initialized', async () => {
        ...
        const tx = await createPoolWithZeroTickInitialized(nft, wallet, tokens[0].address, tokens[2].address)
        await tx.wait()
