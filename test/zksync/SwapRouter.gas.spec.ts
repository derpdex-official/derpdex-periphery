...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture = async (wallets: Wallet[], provider: Provider) => {
    ...
    async function createPoolWETH9(tokenAddress: string) {
      let tx
      tx = await weth9.deposit({ value: liquidity * 2 })
      await tx.wait()
  ...
  beforeEach('intialize feeGrowthGlobals', async () => {
    let tx
    tx = await exactInput([tokens[0].address, tokens[1].address], 1, 0)
    await tx.wait()
    tx = await exactInput([tokens[1].address, tokens[0].address], 1, 0)
    await tx.wait()
    tx = await exactInput([tokens[1].address, tokens[2].address], 1, 0)
    await tx.wait()
    tx = await exactInput([tokens[2].address, tokens[1].address], 1, 0)
    await tx.wait()
    tx = await exactInput([tokens[0].address, weth9.address], 1, 0)
    await tx.wait()
    tx = await exactInput([weth9.address, tokens[0].address], 1, 0)
    await tx.wait()
  })
  ...
  describe('#exactInput', () => {
    ...
    it('2 trades (via router)', async () => {
      let tx
      tx = await weth9.connect(trader).deposit({ value: 3 })
      await tx.wait()
      tx = await weth9.connect(trader).approve(router.address, constants.MaxUint256)
      await tx.wait()
      const swap0 = {
