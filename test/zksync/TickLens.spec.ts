...
import { IUniswapV3Factory, MockTimeNonfungiblePositionManager, TestERC20, TickLensTest } from '../../typechain'
...
describe('TickLens', () => {
  ...
  const nftFixture: Fixture<{
    ...
    for (const token of tokens) {
      const tx = await token.approve(nft.address, constants.MaxUint256)
      await tx.wait()
    }
  ...
  describe('#getPopulatedTicksInWord', () => {
    ...
    async function mint(tickLower: number, tickUpper: number, amountBothDesired: BigNumberish): Promise<number> {
      ...
      const tx = await nft.mint(mintParams)
      await tx.wait()
